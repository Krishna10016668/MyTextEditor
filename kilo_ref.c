...
void editorUpdateRow(erow *row) {
  ...
  for (j = 0; j < row->size; j++) {
    if (row->chars[j] == '\t') {
      row->render[idx++] = ' ';
      while (idx % MyTextEditor_TAB_STOP != 0) row->render[idx++] = ' ';
    } else {
    ...
    if (filerow >= E.numrows) {
      if (E.numrows == 0 && y == E.screenrows / 3) {
        ...
        int welcomelen = snprintf(welcome, sizeof(welcome),
          "MyTextEditor editor -- version %s", MyTextEditor_VERSION);
...
void editorProcessKeypress() {
  static int quit_times = MyTextEditor_QUIT_TIMES;
  ...
  quit_times = MyTextEditor_QUIT_TIMES;
}
